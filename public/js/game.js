document.addEventListener('DOMContentLoaded', function () {
    // Empiezo con los datos del index:
        ...
        const name = decodeURIComponent(parametros.get('nam'));
        const numberPlayers = decodeURIComponent(parametros.get('players'));
        const difficulty = decodeURIComponent(parametros.get('difficulty'));
    ...
    const startGame = () => {
        ...
        state.loop = setInterval(() => {
            ...
            if (!selectors.pause.classList.contains('paused')) {
                state.totalTime++;
            }
    ...
    const flipCard = card => {
        ...
        if (!document.querySelectorAll('.card:not(.flipped)').length) {
            setTimeout(() => {
                ...
                if (state.totalTime > 10 && difficulty === "easy") {
                    selectors.win.innerHTML += `
                <span class="win-text">
                    You are a <span class="highlight">beginner  ${name}</span>
    ...
    const attachEventListeners = () => {
        document.addEventListener('click', event => {
            ...
            if (eventTarget.className.includes('card') && !eventParent.className.includes('flipped')) {
                ...
                console.log(difficulty);
                eventParent.parentElement.style.display = "none"; //Luego soluciono lo del bisabuelo.
                generateGame();
            }
        })
    }
